...
#define DEFAULT_CLASS_NAME	L"MainWindow"
#define IMAGE_WINDOW_CLASS_NAME	L"ImageWindow"
...
CWin32MainWindow::CWin32MainWindow(int x, int y, int width, int height, const char *title)
{
	...
	if (m_ref_count == 0)
	{
		...
		if (!RegisterClass(&wc))
		{
			MessageBox(NULL, L"Failed To Register The MainWindow Class.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
			return;
		}
		...
		if (!RegisterClass(&wc))
		{
			MessageBox(NULL, L"Failed To Register The Image Window Class.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
			return;
		}
	}
	...
	if (!(m_hWnd = CreateWindowEx(dwExStyle,						// Extended Style For The Window
		...
		convertCharArrayToLPCWSTR(title),								// Window Title
	...
	{
		MessageBox(NULL, L"Couldn't create window.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
		return;
	}
...
CWin32MainWindow::~CWin32MainWindow()
{
	...
	for (i = c - 1; i >= 0; i--)
	{
		...
		if (w->m_eType == eImage)
		{
			...
			{
				MessageBox(NULL, L"Release Compatible Device Context Failed.", L"SHUTDOWN ERROR", MB_OK | MB_ICONINFORMATION);
				iw->m_hDCMem = NULL;
			}
...
WIDGET_HANDLE CWin32MainWindow::AddImage(int x, int y, int width, int height, WIDGET_HANDLE parent)
{
	...
	{
		MessageBox(NULL, L"Couldn't create image window.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
		delete iw;
		return 0;
	}
	...
	{
		MessageBox(NULL, L"Couldn't create device context.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
		delete iw;
		return 0;
	}
	...
	{
		MessageBox(NULL, L"Couldn't create compatible device context.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
		delete iw;
		return 0;
	}
...
WIDGET_HANDLE CWin32MainWindow::AddButton(int x, int y, int width, int height, const char *text, WIDGET_HANDLE parent)
{
	...
	if (!(w->m_hWnd = CreateWindowEx(0,								// Extended Style For The Window
		L"button",							// Class Name
		convertCharArrayToLPCWSTR(text),								// Window Title
		WS_CLIPSIBLINGS | WS_CHILD | WS_VISIBLE,				// Required Window Style
		x, y,								// Window Position
		width,								// Calculate Window Width
		height,								// Calculate Window Height
		...
		this)))								// Pass pointer to struct to WM_CREATE
	{
		MessageBox(NULL, L"Couldn't create button.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
...
WIDGET_HANDLE CWin32MainWindow::AddLabel(int x, int y, int width, int height, const char *text, WIDGET_HANDLE parent)
{
	...
	if (!(w->m_hWnd = CreateWindowEx(0,								// Extended Style For The Window
		...
		this)))								// Pass pointer to struct to WM_CREATE
	{
		MessageBox(NULL, L"Couldn't create label.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
...
WIDGET_HANDLE CWin32MainWindow::AddCheckBox(int x, int y, int width, int height, const char *text, bool checked, WIDGET_HANDLE parent)
{
	...
	if (!(w->m_hWnd = CreateWindowEx(0,								// Extended Style For The Window
		...
		this)))								// Pass pointer to struct to WM_CREATE
	{
		MessageBox(NULL, L"Couldn't create check box.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
...
WIDGET_HANDLE CWin32MainWindow::AddTextEdit(int x, int y, int width, int height, const char *text, WIDGET_HANDLE parent)
{
	...
	if (!(w->m_hWnd = CreateWindowEx(WS_EX_STATICEDGE,				// Extended Style For The Window
		...
		this)))								// Pass pointer to struct to WM_CREATE
	{
		MessageBox(NULL, L"Couldn't create text edit.", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
...
WIDGET_HANDLE CWin32MainWindow::AddComboBox(int x, int y, int width, int height, int num_entries, const char **entries, int current_entry, WIDGET_HANDLE parent)
{
	...
	{
		SendMessageW(w->m_hWnd, CB_ADDSTRING, 0, (LPARAM)convertCharArrayToLPCWSTR(entries[i]));
	}
...
WIDGET_HANDLE CWin32MainWindow::AddGLWidget(int x, int y, int width, int height, WIDGET_HANDLE parent)
{
	...
	if (!bResult)
	{
		MessageBox(NULL, L"ERROR: SetPixelFormat failed", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
	...
	if (!glw->m_hGLRC)
	{
		MessageBox(NULL, L"ERROR: couldn't allocate HGLRC", L"ERROR", MB_OK | MB_ICONEXCLAMATION);
...
bool CWin32MainWindow::GetText(WIDGET_HANDLE widget, char *text, int len)
{
	...
	LPTSTR lText = (LPTSTR)VirtualAlloc((LPVOID)NULL, (DWORD)(len + 1), MEM_COMMIT, PAGE_READWRITE);

	if (w->m_eType == eButton || w->m_eType == eCheckBox)
	{
		Button_GetText(w->m_hWnd, lText, len);
		wcstombs(text, lText, len);
	}
	else if (w->m_eType == eLabel)
	{
		Static_GetText(w->m_hWnd, lText, len);
		wcstombs(text, lText, len);
	}
	else if (w->m_eType == eTextEdit)
	{
		GetWindowText(w->m_hWnd, lText, len);
		wcstombs(text, lText, len);
	}
	else
		return false;
...
bool CWin32MainWindow::SetText(WIDGET_HANDLE widget, const char * text)
{
	...
	//LPCWSTR lText = convertCharArrayToLPCWSTR(text);

	if (w->m_eType == eButton || w->m_eType == eCheckBox)
	{
		Button_SetText(w->m_hWnd, convertCharArrayToLPCWSTR(text));
	}
	else if (w->m_eType == eLabel)
	{
		Static_SetText(w->m_hWnd, convertCharArrayToLPCWSTR(text));
	}
	else if (w->m_eType == eTextEdit)
	{
		SetWindowText(w->m_hWnd, convertCharArrayToLPCWSTR(text));
	}
	else
		return false;
...
//			if (!iw->m_hBmp)
//			{
//				MessageBox(NULL, L"Couldn't create DIB." , L"ERROR", MB_OK | MB_ICONEXCLAMATION);
//			}
//			
...
//			if (!iw->m_hBmpOld)
//			{
//				MessageBox(NULL, L"Couldn't Select Object." , L"ERROR", MB_OK | MB_ICONEXCLAMATION);
//			}
//
